...
export default function IdCardModal({ organization, teacher, profile, isOpen, onClose }: IdCardModalProps) {
  ...
  const handlePrint = () => {
    ...
    if (printWindow) {
      printWindow.document.write(`
        ...
        <html>
          <head>
            <title>Print ID Card - ${teacher.firstName} ${teacher.lastName}</title>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm overflow-y-auto">
      <div className="bg-white rounded-3xl shadow-xl w-full max-w-2xl overflow-hidden flex flex-col my-8">
          ...
          <div ref={printRef} className="hidden">
            ...
            <div className="id-card bg-white">
              <div className="w-full h-full flex flex-col relative bg-white">
                <div className="bg-purple-600 text-white px-4 py-2 flex items-center justify-between shrink-0 h-10">
                  <span className="font-bold text-sm truncate max-w-[150px]">{organization.name}</span>
                  <span className="text-[10px] font-medium uppercase tracking-wider opacity-90">Staff ID</span>
                </div>
                <div className="flex-1 flex p-3 gap-3">
                  <div className="w-[1.2in] h-[1.2in] shrink-0 rounded-lg overflow-hidden bg-[#f5f5f5] border-2 border-white shadow-sm flex items-center justify-center">
                    {profile?.photoURL ? (
                      <img src={profile.photoURL} alt="Teacher" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                    ) : (
                      <UserCircle className="w-12 h-12 text-[#9e9e9e]" />
                    )}
                  </div>
                  <div className="flex-1 flex flex-col justify-center min-w-0">
                    <h3 className="font-bold text-lg leading-tight text-[#1a1a1a] truncate">{teacher.firstName}</h3>
                    <h3 className="font-bold text-lg leading-tight text-[#1a1a1a] truncate mb-1">{teacher.lastName}</h3>
                    <div className="space-y-0.5 mt-1">
                      <p className="text-[10px] text-[#4a4a4a] flex items-center gap-1"><span className="font-bold text-[#9e9e9e] w-12">ID:</span><span className="font-mono font-medium truncate">{teacher.teacherId}</span></p>
                      <p className="text-[10px] text-[#4a4a4a] flex items-center gap-1"><span className="font-bold text-[#9e9e9e] w-12">Role:</span><span className="font-medium truncate">Teacher</span></p>
                      <p className="text-[10px] text-[#4a4a4a] flex items-center gap-1"><span className="font-bold text-[#9e9e9e] w-12">Hired:</span><span className="font-medium truncate">{new Date(teacher.hireDate).toLocaleDateString()}</span></p>
                    </div>
                  </div>
                </div>
                <div className="absolute bottom-2 right-2 flex items-end justify-end">
                  <div className="bg-white p-1 rounded shadow-sm border border-[#e5e5e5]">
                    <QRCode value={`TEACHER:${organization.id}:${teacher.id}:${teacher.teacherId}`} size={40} level="L" />
                  </div>
                </div>
                <div className="absolute top-10 right-0 w-32 h-32 bg-purple-600/5 rounded-full -mr-16 -mt-16 pointer-events-none" />
                <div className="absolute bottom-0 left-0 w-24 h-24 bg-purple-600/5 rounded-full -ml-12 -mb-12 pointer-events-none" />
              </div>
            </div>
            ...
            <div className="id-card bg-white">
              <div className="w-full h-full flex flex-col relative bg-white p-4">
                ...
                <div className="mt-auto pt-2 border-t border-[#e5e5e5] flex justify-between items-center">
                  ...
                  <span className="text-[8px] text-[#9e9e9e] font-mono">{teacher.id.slice(0, 8)}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="p-6 border-t border-[#e5e5e5] flex justify-end gap-3 bg-white">
          <button
            onClick={onClose}
