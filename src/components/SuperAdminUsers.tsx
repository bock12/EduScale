...
export default function SuperAdminUsers({ setActiveOrgId }: SuperAdminUsersProps) {
  ...
  const handleViewDashboard = (user: UserProfile) => {
    if (user.role === 'super_admin') {
      if (setActiveOrgId) setActiveOrgId(null);
      navigate('/');
    } else if (user.organizationId) {
      if (setActiveOrgId) setActiveOrgId(user.organizationId);
      navigate(`/?org=${user.organizationId}`);
    } else {
      if (setActiveOrgId) setActiveOrgId(null);
      navigate('/');
    }
  ...
  return (
    <div className="space-y-6 md:space-y-8 pb-8">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl md:text-3xl font-bold tracking-tight mb-2">User Management</h2>
          <p className="text-[#9e9e9e] text-sm md:text-base">Manage all users across the platform.</p>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[#f9f9f9] border-b border-[#e5e5e5]">
                ...
                <th className="px-6 py-4 text-xs font-bold uppercase tracking-widest text-[#9e9e9e]">Organization</th>
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2 text-sm text-[#4a4a4a]">
                        <SchoolIcon className="w-4 h-4 text-[#9e9e9e]" />
                        <span>{organizations[user.organizationId] || user.organizationId || 'N/A'}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2 text-sm text-[#4a4a4a]">
                                ...
                                >
                                  <div className="p-2">
                                    <button
                                      ...
                                      className="w-full flex items-center gap-3 px-4 py-2.5 text-sm font-bold text-[#4a4a4a] hover:bg-blue-50 hover:text-blue-600 rounded-xl transition-all"
                                    >
                                    ...
                                    <button
                                      onClick={() => {
                                        handleUpdateRole(user.id!, user.role === 'super_admin' ? 'school_admin' : 'super_admin');
                                        setActiveDropdown(null);
                                      }}
                                      className="w-full flex items-center gap-3 px-4 py-2.5 text-sm font-bold text-[#4a4a4a] hover:bg-purple-50 hover:text-purple-600 rounded-xl transition-all"
                                    >
            ...
            {filteredUsers.map((user) => (
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="bg-[#f9f9f9] p-3 rounded-2xl">
                    <p className="text-[10px] font-bold text-[#9e9e9e] uppercase tracking-widest mb-1">Organization</p>
                    <p className="text-xs font-bold truncate">{organizations[user.organizationId] || user.organizationId || 'N/A'}</p>
                  </div>
                </div>
              </motion.div>
            ))}
